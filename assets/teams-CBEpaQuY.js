var e=`import { LfArrowRight, LfArrowUpRightFromSquare } from "@legalforce/aegis-icons";
import {
  Banner,
  Button,
  ButtonGroup,
            ...
            >
              <Banner
                ...
                action={
                  <Button
                    size="small"
                    variant="subtle"
                    disabled={reconsentPending}
                    loading={reconsentPending}
                    onClick={() => {
                      setReconsentPending(true);
                      window.setTimeout(() => setReconsentPending(false), 900);
                    }}
                    trailing={LfArrowRight}
                  >
                    追加画面を開く
                  </Button>
                }
              >
                <Text whiteSpace="pre-wrap">
                  {
                    "「LegalOnの画面上からTeamsへ返信」機能が新たに追加されました。\\nご利用には、Microsoft管理者による権限の追加が必要です。"
                  }
                </Text>
              </Banner>
