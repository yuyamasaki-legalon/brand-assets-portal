...
<span color="red">**\`{'\\n'}\` と \`whiteSpace="pre-line"\` で改行を作らないこと。レイアウトコンポーネントで分割すること。**</span>

\`<Text>\` の中に \`{'\\n'}\` を入れて \`whiteSpace="pre-line"\` で改行を表示するパターンは、以下の問題があります：

- **i18n と相性が悪い**: 翻訳ファイルや翻訳ツールで改行コードが消えたり、言語によって改行位置が変わるべきケースに対応できない
- **インライン要素と段落の混在**: \`<Text>\` 内に \`<Link>\` や \`<br />\` を含めるとレイアウトが壊れやすい
- **アクセシビリティ**: 視覚的な改行と文書構造（段落、リスト）が一致しない
- **CSS で制御できない**: ハードコードされた改行は responsive breakpoint で調整できない

### ❌ 悪い例（プロトタイプでよく発生）

\`\`\`tsx
// 改行コードを使って Text 内で段落を作っている
<Text variant="body.medium" whiteSpace="pre-line">
  {t({ key: "dialogDescription" })}
  {'\\n'}
  <Link href={settingsUrl} trailing={LfArrowUpRightFromSquare}>
    {t({ key: "settingsLinkLabel" })}
  </Link>
</Text>
\`\`\`
...
\`\`\`tsx
// Text と Link を別要素にし、レイアウトコンテナで縦並びにする
...
| シナリオ | 採用すべきパターン |
|---------|------------------|
| 段落と段落の分離 | \`<Text as="p">\` を複数個並べて、親で flex/Stack |
| テキスト内の意味的改行 | i18n の翻訳キー側で \`\\n\` を含め、\`whiteSpace="pre-line"\` を使う |
| テキストの後に Link / Button を続ける | \`<Text>\` と \`<Link>\` / \`<Button>\` を別要素にして親で gap |
| 装飾的な改行（読みやすさのため） | レイアウト側で width/wrap を制御し、改行コードに頼らない |

\`whiteSpace="pre-line"\` 自体は禁止ではないが、**インライン要素（Link, Button, Icon）を含む \`<Text>\` で使うのは禁止**。純粋なテキスト内に翻訳文由来の改行を保持する目的のみに使うこと。
