...
const BrandAssetPortal = () => {
  ...
  const handleDownload = async (asset: Asset | null) => {
    ...
    // Always prefer the canonical Google Drive file when we have a driveId.
    // Shared icons only fall back to generated output if no Drive file exists.
    if (asset.driveId) {
    ...
    if (asset.downloadUrl) {
      triggerFileDownload(asset.downloadUrl, getAssetDownloadName(asset));
      return;
    }
