...
- Add content で追加したコンポーネントはデフォルトで各エリアに \`{/* ComponentName, ... */}\` コメントとして列挙される
- **「Add content のコンポーネントを含める」チェックボックス**を ON にすると、コメントの代わりに実際の Aegis コンポーネント JSX が出力され、\`import\` 文も自動補完される（詳細: [component-jsx-generation.md](./component-jsx-generation.md)）
