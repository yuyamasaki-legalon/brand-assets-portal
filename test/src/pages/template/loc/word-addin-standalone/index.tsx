...
import type { Document } from "../word-addin/mock/data";
import {
  ...
  PLAYBOOK_LIST,
  STORAGE_OPTIONS,
} from "../word-addin/mock/data";
import { type WordAddinTab, WordAddinTabNavigation } from "../word-addin/WordAddinTabNavigation";

type ViewType = "review" | "riskCheck";

/**
 * Word Add-in スタンドアロン表示ページ
 *
 * addin-debug.html の iframe 内に表示するタスクペイン UI。
 * 既存 word-addin テンプレートのペイン内コンテンツ（ヘッダー + ビュー + ドロワー）を
 * PageLayout > PageLayoutContent で直接描画する。
 *
 * Aegis Provider のデフォルト min-inline-size: 960px を 300px に上書きし、
 * 実際の Word タスクペイン幅（300-600px）で動作するようにしている。
 */
const WordAddinStandalone = () => {
...
export default WordAddinStandalone;
