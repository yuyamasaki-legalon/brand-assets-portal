...
type FileCategory = "contract" | "attachment" | "reference";
...
const mainContractFile = linkedFiles.find((file) => file.category === "contract");
const attachmentFiles = linkedFiles.filter((file) => file.category === "attachment");
const referenceFiles = linkedFiles.filter((file) => file.category === "reference");
...
const inlineStyles: Record<string, CSSProperties> = {
  ...
  contractDocumentCard: {
    padding: "var(--aegis-space-medium)",
    border: "2px solid var(--aegis-color-border-brand)",
    borderRadius: "var(--aegis-radius-medium)",
    backgroundColor: "var(--aegis-color-surface-default)",
    display: "flex",
    alignItems: "center",
...
export const CaseDetailLinkedFiles = () => {
  ...
  const renderPaneContent = () => {
    ...
    if (paneType === "linked-file") {
      const renderFileItem = (file: LinkedFile) => (
        <div
          key={file.name}
          style={{
            display: "flex",
            ...
            border:
              file.category === "contract"
                ? "2px solid var(--aegis-color-border-brand)"
                : "1px solid var(--aegis-color-border-default)",
            borderRadius: "var(--aegis-radius-medium)",
          }}
        >
          <Icon size="medium" color={file.category === "contract" ? "brand" : undefined}>
            <LfFile />
          </Icon>
          <div
            style={{
          ...
          <div style={{ marginLeft: "auto", flexShrink: 0 }}>
            <Button size="small" variant="subtle" onClick={() => file.url && window.open(file.url, "_blank")}>
              開く
            </Button>
          </div>
        </div>
        ...
        <>
          ...
          <PageLayoutBody>
            <div style={inlineStyles.paneBody}>
              {mainContractFile ? (
                <div style={inlineStyles.fileCategorySection}>
                  <div style={inlineStyles.fileCategoryHeader}>
                    <Text variant="label.small.bold" color="brand">
                      契約書本体
                    </Text>
                  </div>
                  {renderFileItem(mainContractFile)}
                </div>
              ) : null}
              ...
              {attachmentFiles.length > 0 ? (
                <div style={inlineStyles.fileCategorySection}>
                  <div style={inlineStyles.fileCategoryHeader}>
                    <Text variant="label.small.bold">添付書類</Text>
                  </div>
                  {attachmentFiles.map(renderFileItem)}
                </div>
              ) : null}
              ...
              {referenceFiles.length > 0 ? (
                <div style={inlineStyles.fileCategorySection}>
                  <div style={inlineStyles.fileCategoryHeader}>
                    <Text variant="label.small.bold">参考書類</Text>
                  </div>
                  {referenceFiles.map(renderFileItem)}
                </div>
              ) : null}
            </div>
          </PageLayoutBody>
        ...
        <PageLayoutContent maxWidth="medium">
          <PageLayoutBody>
            <div style={inlineStyles.pageBody}>
              ...
              {mainContractFile ? (
                <section style={inlineStyles.contractDocumentSection}>
                  <div style={inlineStyles.fileCategoryHeader}>
                    <Icon size="small">
                      <LfFile />
                    </Icon>
                    ...
                    <Tooltip title="この案件の主要な契約書ファイルです" placement="top">
                  ...
                  <div style={inlineStyles.contractDocumentCard}>
                    <Icon size="large" color="brand">
                      <LfFile />
                    </Icon>
                    <div style={inlineStyles.contractDocumentInfo}>
                      <Text variant="body.medium.bold" numberOfLines={1}>
                        {mainContractFile.name}
                      </Text>
                      <Text variant="body.small" color="subtle">
                        最終更新 {mainContractFile.updatedAt}
                      </Text>
                    </div>
                    <div style={inlineStyles.contractDocumentActions}>
                      <Button
                        size="small"
                        variant="subtle"
                        leading={LfArrowUpRightFromSquare}
                        onClick={() => window.open(mainContractFile.url, "_blank")}
                      >
                        開く
                      </Button>
                      <Button size="small" variant="gutterless" onClick={() => handleSelectPane("linked-file")}>
                        すべてのファイルを見る
                      </Button>
                    </div>
                  </div>
                      ...
                      <div style={inlineStyles.card}>
                        ...
                        >
                          {timelineEvents
                            ...
                            .map((mail) => (
                              <div key={mail.id} style={inlineStyles.externalCard}>
