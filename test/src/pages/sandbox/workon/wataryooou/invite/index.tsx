...
const dataTableColumns: DataTableColumnDef<Employee, string>[] = [
  {
    id: "employeeNumber",
    name: "従業員番号",
    getValue: (row): string => row.employeeNumber,
  },
  {
    id: "name",
    name: "従業員名",
    getValue: (row): string => row.name,
    renderCell: ({ value }) => <DataTableCell leading={<Avatar name={value} size="xSmall" />}>{value}</DataTableCell>,
  },
  {
    id: "employmentType",
    name: "雇用形態",
    getValue: (row): string => row.employmentType,
  },
  {
    id: "employmentStatus",
    name: "在籍状況",
    getValue: (row): string => row.employmentStatus,
  },
  {
    id: "email",
    name: "社用メールアドレス",
    getValue: (row): string => row.email,
  },
  {
    id: "invitationStatus",
    name: "ステータス",
    getValue: (row): string => row.invitationStatus,
    renderCell: ({ row }) => {
      const status = statusConfig[row.invitationStatus];
      return (
        <DataTableCell>
...
export function InvitePage() {
  const [selectedRows, setSelectedRows] = useState<string[]>(["2", "3", "5"]);
        ...
        <PageLayout>
          <PageLayoutContent>
            <PageLayoutHeader>
              ...
              >
                <ContentHeader.Title>従業員を招待</ContentHeader.Title>
              </ContentHeader>
            </PageLayoutHeader>
            <PageLayoutBody>
              <div style={{ display: "flex", flexDirection: "column", gap: "var(--aegis-space-medium)" }}>
                ...
                <DataTable
                  ...
                  defaultColumnSizing={{ actions: 120 }}
                />
